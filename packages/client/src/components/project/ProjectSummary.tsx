...
import { BarChart3, Users, AlertTriangle, CheckCircle2, Clock, Target } from "lucide-react";
...
export default function ProjectSummary() {
  ...
  useEffect(() => {
    api.get<ProjectSummaryItem[]>("/projects/summary").then(res => {
  ...
  return (
    <div className="p-3 sm:p-4 space-y-3 overflow-y-auto h-full">
      <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
        ...
        {t("project.title")} ({projects.length})
      </h2>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-sm font-semibold text-slate-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  {p.name}
                </span>
              </div>
              {p.dDay !== null && (
