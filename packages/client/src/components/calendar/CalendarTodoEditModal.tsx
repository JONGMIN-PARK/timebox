...
interface CalendarTodoEditModalProps {
  open: boolean;
  todo: Todo | null;
  onClose: () => void;
  onSave: (id: number, values: CalendarTodoAddValues) => Promise<void>;
}

function effectiveStatus(todo: Todo): TodoStatus {
  if (todo.status) return todo.status;
  return todo.completed ? "completed" : "active";
}

export default function CalendarTodoEditModal({ open, todo, onClose, onSave }: CalendarTodoEditModalProps) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    if (d) {
      setDueDateOnly(d.slice(0, 10));
      setDueTime(d.includes("T") ? d.slice(11, 16) : "");
    } else {
      setDueDateOnly(new Date().toISOString().slice(0, 10));
      setDueTime("");
    }
