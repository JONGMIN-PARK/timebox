...
- **팀 협업:** 팀 그룹 기반 프로젝트 관리, 칸반 보드, 게시판, 채팅, 메시지함
...
- **개인 사용자**: 캘린더, 타임박스, 투두, 디데이, 리마인더, 파일 보관함
- **팀 사용자**: 개인 기능 + 프로젝트(칸반/현황판), 게시판, 자료실, 채팅
- **관리자**: 팀 그룹 관리, 사용자 관리, 프로젝트 생성/관리
- 여러 디바이스에서 접근 (PC, 모바일, 태블릿)
- 인증: JWT 기반 다중 사용자 시스템 (관리자 승인 가입)
...
| 기능 | 설명 | 우선순위 |
|------|------|----------|
| 팀 그룹 관리 | 관리자가 그룹 생성, 사용자 배정/해제 | P0 |
| 프로젝트 관리 | 생성/수정/삭제, 시작일/목표일/D-Day, 문서 | P0 |
| 칸반 보드 | 5단계(Backlog→Done), 드래그, 담당자 배정 | P0 |
| 프로젝트 현황판 | 진행률, D-Day, 주간 통계, 멤버별 현황, 활동 로그 | P0 |
| 게시판 | 공지/토론/질문 카테고리, 댓글, 고정글 | P1 |
| 공유 자료실 | 폴더 구조, 파일 업로드/다운로드 | P1 |
| 팀 채팅 | 프로젝트별 채팅, 폴링 방식 | P1 |
| 업무 전달 | 태스크 이관 요청/수락/거절 워크플로우 | P1 |
| 메시지함 | 사용자 간 메시지, 태스크 할당 자동 알림 | P0 |
| 온라인 상태 | 사이드바에 접속 중인 팀 멤버 표시 | P2 |

### 3.9 텔레그램 봇 확장 (구현 완료)

| 기능 | 설명 | 우선순위 |
|------|------|----------|
| 개별 사용자 연동 | /link CODE로 각자 텔레그램 연결 | P0 |
| 프로젝트 현황 | /project — 내 프로젝트 진행률, D-Day | P1 |
| 내 태스크 | /mytasks — 할당된 활성 태스크 목록 | P1 |
| 메시지 확인 | /inbox — 안읽은 메시지 목록 | P1 |
| 메시지 발송 | /msg @user 내용 — 텔레그램에서 메시지 보내기 | P2 |
| 명령어 자동완성 | / 입력 시 17개 명령어 메뉴 표시 | P1 |
